...
function PaletteDialog({ onClose, projects, email, github, home }: PaletteProps) {
  ...
  const actions = useMemo<Action[]>(() => {
    const go = (id: string) => () => (home ? (location.hash = id) : router.push(`/#${id}`));
    return [
      ...
      ...projects.map((p) => ({
        ...
        run: () => router.push(`/projets/${encodeURIComponent(p.slug)}`),
      })),
      {
        id: "copy-email",
      ...
      ...(github
        ? [{
            id: "github",
            label: "Ouvrir mon GitHub",
            group: "Actions",
            icon: <ExternalLink className="size-4" />,
            run: () => window.open(github, "_blank", "noopener"),
          }]
        : []),
      { id: "theme", label: "Changer de thème", group: "Actions", icon: <Moon className="size-4" />, run: toggleTheme },
    ];
  }, [projects, email, github, home, router]);
                  ...
                  <li key={a.id}>
                    ...
                    >
                      ...
                      <span className="flex-1 truncate">
                        {a.id === "copy-email" && copied ? "Adresse copiée ✓" : a.label}
                      </span>
